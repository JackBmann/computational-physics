...
# define the acceleration from Newton's second law
def acc(earthpos):
    r = mag(earthpos - sun.pos)
    return -G*sun.mass*(earthpos - sun.pos)/r**3


# define our RK4 algorithm
def rk4(earth):
    k1v = acc(earth.pos) * h
    k1x = earth.vel * h
    k2v = (acc(earth.pos) + k1x/2.0) * h
    k2x = (earth.vel + k1v/2.0) * h
    k3v = (acc(earth.pos) + k2x/2.0) * h
    k3x = (earth.vel + k2v/2.0) * h
    k4v = (acc(earth.pos) + k3x) * h
    k4x = (earth.vel + k3v) * h

    earth.vel += (1/6.0) * (k1v + 2.0*k2v + 2.0*k3v + k4v)
    earth.pos += (1/6.0) * (k1x + 2.0*k2x + 2.0*k3x + k4x)
...
while True:
    ...
    rk4(earth)  # call our Runge-Kutta 4th order and pass earth properties
